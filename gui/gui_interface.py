...
class JobMatchApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.seed_func = set_all_seeds

        self.create_widgets()

    def create_widgets(self):
        # Drag-and-drop area for instructors file
        self.instructor_label = tk.Label(self, text="Drop Instructors File Here", width=50, height=5, bg='lightgrey')
        self.instructor_label.pack(pady=10)
        self.instructor_label.drop_target_register(DND_FILES)
        self.instructor_label.dnd_bind('<<Drop>>', self.load_instructor_file)

        # Drag-and-drop area for courses file
        self.course_label = tk.Label(self, text="Drop Courses File Here", width=50, height=5, bg='lightgrey')
        ...
        method_label = tk.Label(self, text="Select Matching Method:")
        method_label.pack(pady=5)

        method_options = ["bipartite_matching", "stable_marriage", "linear_programming"]
        method_menu = tk.OptionMenu(self, self.selected_method, *method_options)
        method_menu.pack(pady=10)

        # Dropdown for match type selection
        match_type_label = tk.Label(self, text="Select Match Type to View:")
        match_type_label.pack(pady=5)

        match_type_options = ["Instructor Matches", "Course Matches"]
        match_type_menu = tk.OptionMenu(self, self.match_type, *match_type_options)
        match_type_menu.pack(pady=10)

        # Button to run the matching algorithm
        self.run_button = tk.Button(self, text="Run Matching", command=self.run_matching, width=15)
        self.run_button.pack(pady=5)

        # Button to view the selected match type
        self.view_button = tk.Button(self, text="Print Results", command=self.view_matches, width=15)
    ...
    def load_instructor_file(self, event):
        ...
        if file_path and file_path != self.instructor_file:
            ...
            try:
                ...
                if self.instructors:
                    tk.messagebox.showinfo("File Loaded", f"Instructor file loaded: {self.instructor_file}")
    ...
    def load_course_file(self, event):
        ...
        if file_path and file_path != self.course_file:
            ...
            try:
                ...
                if self.courses:
                    tk.messagebox.showinfo("File Loaded", f"Course file loaded: {self.course_file}")
            except Exception as e:
    ...
    def run_matching(self):
        ...
        try:
            ...
            tk.messagebox.showinfo("Matching Results", "Matching completed successfully! Use the dropdown and 'Print Results' button to view results.")
    ...
    def view_matches(self):
        ...
        if match_type == "Instructor Matches":
            result_text = self.get_match_results_text(self.matching_results[0])
        elif match_type == "Course Matches":
            result_text = self.get_match_results_text(self.matching_results[1])

        self.display_results_popup(result_text)

    def get_match_results_text(self, results):
        result_lines = []
    ...
    def display_results_popup(self, text):
        ...
        text_area = tk.Text(result_popup, wrap=tk.WORD, width=80, height=20)
