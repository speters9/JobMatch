...
#                            course_capacities: Dict[str, int]) -> Tuple[Dict[str, Optional[str]], Dict[str, List[str]]]:
...
#     instructor_assignments = {instructor: None for instructor in instructor_preferences}
...
#     # Create a list to track free instructors (those not yet assigned)
#     free_instructors = list(instructor_preferences.keys())

#     # While there are free instructors who have not exhausted their preferences
...
#             current_proposals[instructor] += 1  # Move to the next preference

#             # print(f"{instructor} proposes to {preferred_course}")

#             if len(course_assignments[preferred_course]) < course_capacities[preferred_course]:
#                 # If the course has available slots, assign the instructor
#                 course_assignments[preferred_course].append(instructor)
#                 instructor_assignments[instructor] = preferred_course
#                 # print(f"{instructor} matched with {preferred_course}")
#             else:
#                 # If the course is full, check if this instructor is better than the worst match
#                 worst_instructor = min(course_assignments[preferred_course], key=lambda x: next(
#                     p.rank for p in instructor_preferences[x] if p.course == preferred_course))

#                 current_instructor_rank = next(p.rank for p in instructor_preferences[instructor] if p.course == preferred_course)
#                 worst_instructor_rank = next(p.rank for p in instructor_preferences[worst_instructor] if p.course == preferred_course)

#                 if current_instructor_rank < worst_instructor_rank:
#                     # Replace the worst instructor with the current instructor
#                     course_assignments[preferred_course].remove(worst_instructor)
#                     course_assignments[preferred_course].append(instructor)
#                     instructor_assignments[instructor] = preferred_course
#                     instructor_assignments[worst_instructor] = None
#                     free_instructors.append(worst_instructor)
#                     # print(f"{instructor} replaced {worst_instructor} in {preferred_course}")
#                 else:
#                     # The current instructor remains free
#                     free_instructors.append(instructor)

#     # Print the rank of each course assignment according to instructor preferences
#     for instructor, course in instructor_assignments.items():
#         if course:
#             # Convert the list of preferences to a dictionary for easy lookup
#             pref_dict = {pref.course: pref.rank for pref in instructor_preferences[instructor]}
#             rank = pref_dict.get(course, len(course_capacities))
#             #print(f"{instructor} is assigned to {course} (Rank: {rank})")
